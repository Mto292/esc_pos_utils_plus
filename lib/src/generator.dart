...
class Generator {
  ...
  List<int> _toRasterFormat(Image imgSrc) {
    // Orijinal görselin kopyasını oluştur
    ...
    final int rowPadding = (widthPx % 8 == 0) ? 0 : 8 - (widthPx % 8);

    if (rowPadding > 0) {
      final List<int> paddedBytes = [];

      for (int y = 0; y < heightPx; y++) {
        final int rowStart = y * widthPx;
        final int rowEnd = rowStart + widthPx;

        paddedBytes.addAll(oneChannelBytes.sublist(rowStart, rowEnd));
        paddedBytes.addAll(List<int>.filled(rowPadding, 0));
      }
